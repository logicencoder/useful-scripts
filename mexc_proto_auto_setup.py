...
def complete_auto_setup():
    ...
    os.makedirs('mexc_api_schema_from_github', exist_ok=True)
    ...
    for proto_file in proto_files:
        try:
            ...
            if response.status_code == 200:
                with open(f'mexc_api_schema_from_github/{proto_file}', 'w', encoding='utf-8') as f:
    ...
    for proto_file in downloaded_files:
        if os.path.exists(f'mexc_api_schema_from_github/{proto_file}'):
            print(f"  🔧 Generating code for {proto_file}...")
            
            cmd = ['protoc', f'--proto_path=mexc_api_schema_from_github', f'--python_out=generated_proto']
            ...
            try:
                ...
                if result.returncode == 0:
                    ...
                    if needs_experimental:
                        cmd_no_exp = ['protoc', f'--proto_path=mexc_api_schema_from_github', f'--python_out=generated_proto', proto_file]
...
"""
    ...
    with open('config/protobuf_setup_status.json', 'w') as f:
