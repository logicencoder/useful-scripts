        ...
        if message_type == aiohttp.WSMsgType.TEXT:
            ...
            if "code" in data and data["code"] == 0:
                if data.get("msg") == "PONG":
                    ...
                    print(f"✅ Server response: {data.get('msg', 'Unknown')}")
                    return
            ...
            for trade in trades:
                try:
                    ...
                    print(f"❌ Error processing trade: {e}, trade data: {trade}")
    ...
    try:
        ...
        async with aiohttp.ClientSession(timeout=timeout) as session:
            async with session.ws_connect(
                ...
                print(f"📡 Starting persistent connection for {symbol}")
                ...
                while not stop_flag:
                    try:
                        ...
                        if message.type in (aiohttp.WSMsgType.CLOSED, aiohttp.WSMsgType.ERROR):
                            print(f"❌ WebSocket closed or error: {message.type}")
                            if not stop_flag:
        ...
        # Cleanup
        print(f"🧹 Cleaning up connection...")
...
async def main():
    ...
    print(f"📋 This script will maintain ONE connection and NOT create multiple listen keys")
    ...
    try:
        ...
        if attempt >= max_attempts:
            print(f"❌ Max connection attempts ({max_attempts}) reached. Stopping.")
        ...
        stop_flag = True
        print(f"✅ Script terminated gracefully")
