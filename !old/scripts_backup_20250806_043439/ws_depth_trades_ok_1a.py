...
def decode_protobuf_message(data):
    ...
    try:
        # Import protobuf for basic decoding
        ...
        while i < len(data):
            ...
            if i + 1 < len(data):
                length = data[i]
                if 10 <= length <= 100 and i + length + 1 <= len(data):  # Reasonable string length
                    try:
        ...
        return {
            'hex': hex_str[:100] + "..." if len(hex_str) > 100 else hex_str,
        ...
        return {
            'error': str(e),
            'hex': data.hex()[:100] + "..." if len(data) > 100 else data.hex(),
            'size': len(data)
        }

def extract_numbers_from_protobuf(data):
    """Extract potential price/quantity numbers from protobuf data"""
    numbers = []
    
    try:
        # Look for varint and fixed-size numbers
        i = 0
        while i < len(data) - 8:
            # Try to decode as double (8 bytes)
            if i + 8 <= len(data):
                try:
                    import struct
                    double_val = struct.unpack('<d', data[i:i+8])[0]
                    if 0.000001 < abs(double_val) < 1000000:  # Reasonable price/quantity range
                        numbers.append(f"{double_val:.8f}")
                except:
                    pass
            
            # Try to decode as float (4 bytes)
            if i + 4 <= len(data):
                try:
                    float_val = struct.unpack('<f', data[i:i+4])[0]
                    if 0.000001 < abs(float_val) < 1000000:  # Reasonable price/quantity range
                        numbers.append(f"{float_val:.8f}")
                except:
                    pass
    ...
    numbers.sort(key=lambda x: float(x), reverse=True)
    
    return numbers[:20]  # Return top 20 numbers
