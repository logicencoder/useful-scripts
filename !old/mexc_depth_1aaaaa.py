...
<head>
    ...
    <style>
        ...
        .arbitrage-details { font-size: 0.65rem; }
        .arbitrage-table { overflow-x: auto; }
        .detail-row { border-bottom: 1px solid rgba(255,255,255,0.1); }
...
<body class="bg-gray-900 text-white">
    <div class="container mx-auto p-2 sm:p-4">
        ...
        <div class="bg-gray-800 border border-gray-700 rounded-lg p-2 sm:p-4 mb-4">
            ...
            <!-- Connection Info -->
            <div class="bg-gray-700 border border-gray-600 rounded p-2 mb-2 text-xs">
                <p class="text-center">📡 WebSocket Architecture: 1 connection per exchange (not per coin)</p>
                <p class="text-center">⏱️ Update Rate: MEXC (real-time) | Gate.io (100ms)</p>
            </div>
                ...
                <table class="w-full min-w-full">
                    <thead>
                        <tr class="text-xs border-b border-gray-600">
                            ...
                            <th class="w-1/12">Buy From</th>
                            <th class="w-1/12">Sell To</th>
                            <th class="w-1/12">Profit</th>
                            <th class="w-1/12">ROI %</th>
                            <th class="w-1/12">Coins</th>
                            <th class="w-2/12">Buy Price</th>
                            <th class="w-2/12">Sell Price</th>
                            <th class="w-2/12">Details</th>
                        </tr>
                    </thead>
                    <tbody id="arbitrageTable" class="orderbook-row">
                        <tr>
                            <td class="text-center" colspan="9">Waiting for orderbook data...</td>
                        </tr>
                    </tbody>
                </table>
            </div>
    ...
    <script>
        ...
        function calculateArbitrage(amount, buyExchange, sellExchange, buyOrderbook, sellOrderbook) {
            ...
            let totalCost = 0;
            let buyDetails = [];
            let buyPriceRange = { min: Infinity, max: 0 };
            ...
            for (const ask of asks) {
                ...
                if (remainingAmount >= orderValue) {
                    ...
                    // Update price range
                    buyPriceRange.min = Math.min(buyPriceRange.min, priceFloat);
                    buyPriceRange.max = Math.max(buyPriceRange.max, priceFloat);
                } else {
                    // Buy partial order
            ...
            let sellDetails = [];
            let sellPriceRange = { min: Infinity, max: 0 };
            ...
            for (const bid of bids) {
                ...
                if (remainingCoins >= quantityFloat) {
                    // Sell full order
                    const revenue = priceFloat * quantityFloat;
                    ...
                    // Update price range
                    sellPriceRange.min = Math.min(sellPriceRange.min, priceFloat);
                    sellPriceRange.max = Math.max(sellPriceRange.max, priceFloat);
                } else {
                    // Sell partial order
                    const revenue = priceFloat * remainingCoins;
                    totalRevenue += revenue;
                    sellDetails.push({
                        price: priceFloat,
        ...
        // Format arbitrage details for display in table
        function formatArbitrageDetailsInline(buyDetails, sellDetails) {
            let html = '<div class="arbitrage-details">';
            
            // Show first few buy orders
            html += '<div class="mb-1"><strong>BUY:</strong> ';
            const buyCount = Math.min(3, buyDetails.length);
            for (let i = 0; i < buyCount; i++) {
                const detail = buyDetails[i];
                html += `${detail.price.toFixed(5)}×${detail.quantity.toFixed(4)}=$${detail.total.toFixed(2)}`;
                if (i < buyCount - 1) html += ', ';
            }
            if (buyDetails.length > 3) {
                html += ` (+${buyDetails.length - 3} more)`;
            }
            html += '</div>';
            
            // Show first few sell orders
            html += '<div><strong>SELL:</strong> ';
            const sellCount = Math.min(3, sellDetails.length);
            for (let i = 0; i < sellCount; i++) {
                const detail = sellDetails[i];
                html += `${detail.price.toFixed(5)}×${detail.quantity.toFixed(4)}=$${detail.total.toFixed(2)}`;
                if (i < sellCount - 1) html += ', ';
            }
            if (sellDetails.length > 3) {
                html += ` (+${sellDetails.length - 3} more)`;
            }
            html += '</div>';
        ...
        function updateArbitrageCalculator(symbol) {
            ...
            if (!mexcOrderbook || !gateioOrderbook) {
                arbitrageTable.innerHTML = '<tr><td class="text-center" colspan="9">Waiting for orderbook data...</td></tr>';
                return;
            }
            ...
            amounts.forEach(amount => {
                ...
                if (bestResult.profit > bestProfit) {
                    ...
                    bestOpportunity = {
                        ...
                        buyDetails: bestResult.buyDetails,
                        sellDetails: bestResult.sellDetails,
                        buyPriceRange: bestResult.buyPriceRange,
                        sellPriceRange: bestResult.sellPriceRange
                    };
                }
                ...
                // Format price ranges
                const buyPriceRange = bestResult.buyPriceRange.min === Infinity ? 'N/A' : 
                    `${bestResult.buyPriceRange.min.toFixed(5)}-${bestResult.buyPriceRange.max.toFixed(5)}`;
                const sellPriceRange = bestResult.sellPriceRange.min === Infinity ? 'N/A' : 
                    `${bestResult.sellPriceRange.min.toFixed(5)}-${bestResult.sellPriceRange.max.toFixed(5)}`;
                
                // Format details inline
                const detailsHtml = formatArbitrageDetailsInline(bestResult.buyDetails, bestResult.sellDetails);
                ...
                row.innerHTML = `
                    ...
                    <td class="text-center">${buyPriceRange}</td>
                    <td class="text-center">${sellPriceRange}</td>
            ...
            let maxBuyDetails = [];
            let maxSellDetails = [];
            let maxBuyPriceRange = { min: Infinity, max: 0 };
            let maxSellPriceRange = { min: Infinity, max: 0 };
            ...
            while (mexcBidIndex < mexcBids.length && gateioAskIndex < gateioAsks.length) {
                ...
                if (mexcBidPrice > gateioAskPrice) {
                    ...
                    if (profit > 0) {
                        ...
                        // Add to buy/sell details
                        maxBuyDetails.push({
                            price: gateioAskPrice,
                            quantity: tradeQty,
                            total: gateioAskPrice * tradeQty
                        });
                        
                        maxSellDetails.push({
                            price: mexcBidPrice,
                            quantity: tradeQty,
                            total: mexcBidPrice * tradeQty
                        });
                        
                        // Update price ranges
                        maxBuyPriceRange.min = Math.min(maxBuyPriceRange.min, gateioAskPrice);
                        maxBuyPriceRange.max = Math.max(maxBuyPriceRange.max, gateioAskPrice);
                        maxSellPriceRange.min = Math.min(maxSellPriceRange.min, mexcBidPrice);
                        maxSellPriceRange.max = Math.max(maxSellPriceRange.max, mexcBidPrice);
                    }
            ...
            let maxBuyDetails2 = [];
            let maxSellDetails2 = [];
            let maxBuyPriceRange2 = { min: Infinity, max: 0 };
            let maxSellPriceRange2 = { min: Infinity, max: 0 };
                ...
                if (gateioBidPrice > mexcAskPrice) {
                    ...
                    if (profit > 0) {
                        ...
                        if (!maxProfitDetails || maxProfit2 > maxProfit) {
                            ...
                            // Use the details from this scenario
                            maxBuyDetails = maxBuyDetails2;
                            maxSellDetails = maxSellDetails2;
                            maxBuyPriceRange = maxBuyPriceRange2;
                            maxSellPriceRange = maxSellPriceRange2;
                        }
                        ...
                        maxBuyDetails2.push({
                            price: mexcAskPrice,
                            quantity: tradeQty,
                            total: mexcAskPrice * tradeQty
                        });
                        
                        maxSellDetails2.push({
                            price: gateioBidPrice,
                            quantity: tradeQty,
                            total: gateioBidPrice * tradeQty
                        });
                        
                        // Update price ranges
                        maxBuyPriceRange2.min = Math.min(maxBuyPriceRange2.min, mexcAskPrice);
                        maxBuyPriceRange2.max = Math.max(maxBuyPriceRange2.max, mexcAskPrice);
                        maxSellPriceRange2.min = Math.min(maxSellPriceRange2.min, gateioBidPrice);
                        maxSellPriceRange2.max = Math.max(maxSellPriceRange2.max, gateioBidPrice);
                    }
            ...
            if (maxProfitDetails && maxProfitDetails.maxPossibleProfit > 0) {
                ...
                for (const detail of maxBuyDetails) {
                    totalInvestment += detail.total;
                }
                
                const roiPercent = (maxProfitDetails.maxPossibleProfit / totalInvestment) * 100;
                
                // Format price ranges
                const buyPriceRange = maxBuyPriceRange.min === Infinity ? 'N/A' : 
                    `${maxBuyPriceRange.min.toFixed(5)}-${maxBuyPriceRange.max.toFixed(5)}`;
                const sellPriceRange = maxSellPriceRange.min === Infinity ? 'N/A' : 
                    `${maxSellPriceRange.min.toFixed(5)}-${maxSellPriceRange.max.toFixed(5)}`;
                
                // Format details inline
                const detailsHtml = formatArbitrageDetailsInline(maxBuyDetails, maxSellDetails);
                ...
                row.innerHTML = `
                    ...
                    <td class="${profitClass} text-center font-bold">${profitSign}${roiPercent.toFixed(2)}%</td>
                ...
                row.innerHTML = `
                    <td class="text-center font-bold">MAX</td>
                    <td class="text-center" colspan="8">No arbitrage opportunity available</td>
                `;
                arbitrageTable.appendChild(row);
            }
        }
