...
# FIXED: 4-line debug window + cumulative values + flexible symbol input
...
def get_symbol_selection() -> str:
    ...
    while True:
        try:
            ...
            print(f"Normalized symbol: {user_input} -> {normalized_symbol}")
            
            # Confirm with user
            confirm = input(f"Monitor {normalized_symbol}? (y/n): ").strip().lower()
            if confirm in ['y', 'yes', '']:
                print(f"Selected symbol: {normalized_symbol}")
                return normalized_symbol
            else:
                print("Please enter a different symbol.")
                continue
    ...
    def load_api_keys(self):
        ...
        try:
            ...
            if not self.api_key or not self.api_secret:
                self.log_with_timestamp("❌ Missing keys in mexc_keys.json")
                return False
...
# ============================================================================
# TEXTUAL UI COMPONENTS - FIXED LAYOUT WITH 4-LINE DEBUG AND CUMULATIVE VALUES
# ============================================================================

class OrderbookConnectionPanel(Static):
    """Panel for displaying WebSocket connection status and debug info - FIXED TO 4 LINES."""
...
class OrderbookDisplayPanel(Static):
    """Panel for displaying 20-depth orderbook - SIDE BY SIDE WITH CUMULATIVE VALUES."""
    ...
    def compose(self) -> ComposeResult:
        """Create the orderbook display layout - SIDE BY SIDE WITH CUMULATIVE."""
    ...
    def on_mount(self) -> None:
        """Initialize orderbook tables with cumulative columns."""
        # Setup bids table with cumulative columns
        bids_table = self.query_one("#bids-table")
        bids_table.cursor_type = "none"
        bids_table.add_columns("Price", "Quantity", "Total", "Cum.Qty", "Cum.USD")
        
        # Setup asks table with cumulative columns
        asks_table = self.query_one("#asks-table")
        asks_table.cursor_type = "none"
        asks_table.add_columns("Price", "Quantity", "Total", "Cum.Qty", "Cum.USD")

    def update_orderbook(self, orderbook: dict) -> None:
        """Update orderbook display with 20-depth data and cumulative values."""
        if not orderbook:
            return
        ...
        try:
            ...
            # Update bids table (buy orders - highest price first) with cumulative values
            bids_table = self.query_one("#bids-table")
            bids_table.clear()
            ...
            # Update asks table (sell orders - lowest price first) with cumulative values
            asks_table = self.query_one("#asks-table")
            asks_table.clear()
...
class MEXCOrderbookAppDirectGeneratedProto(App):
    """Main application class - DIRECT GENERATED_PROTO ORDERBOOK VERSION WITH FIXED LAYOUT."""
    ...
    def compose(self) -> ComposeResult:
        """Create main layout with fixed proportions."""
...
# ============================================================================
# MAIN PROGRAM - DIRECT GENERATED_PROTO ORDERBOOK VERSION WITH FIXES
# ============================================================================

def main():
    """Main program entry for DIRECT GENERATED_PROTO orderbook monitor with fixes."""
    global should_exit, selected_symbol
    
    try:
        print("🔥 MEXC DIRECT GENERATED_PROTO ORDERBOOK MONITOR - FIXED VERSION")
        print("🎯 NO FALLBACKS - STRAIGHT TO GENERATED_PROTO FOLDER")
        print("✅ FIXED: 4-line debug window + cumulative values + flexible symbol input")
        print("="*70)
        ...
        # Setup logging
        setup_logging()
        logging.info("Starting DIRECT GENERATED_PROTO MEXC Orderbook Monitor - FIXED VERSION")
        ...
        print("Starting GUI with FIXED layout (4-line debug + cumulative values)...")
        
        # Update terminal title
        set_terminal_title(f"MEXC Orderbook Monitor - FIXED - {selected_symbol}")
