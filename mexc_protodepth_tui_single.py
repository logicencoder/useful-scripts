...
# FIXED: 30/70 layout + 4-line debug + cumulative values + weighted average price + gap
...
def load_generated_proto_protobuf():
    """Load orderbook protobuf from shared generated_proto/ via mexc_proto_loader."""
    print("📦 IMPORTING ORDERBOOK PROTOBUF MODULES FROM generated_proto/ ...")
...
def normalize_symbol(user_input: str) -> str:
    """Normalize symbol input to handle case insensitive and auto-add USDT."""
    try:
        # Convert to uppercase and strip whitespace
        symbol = user_input.strip().upper()
        
        # If symbol doesn't end with a known quote currency, add USDT
        quote_currencies = ['USDT', 'USDC', 'BTC', 'ETH', 'BNB']
        
        # Check if it already ends with a quote currency
        has_quote = any(symbol.endswith(quote) for quote in quote_currencies)
        
        if not has_quote:
            # Auto-add USDT for symbols like "DNX", "BTC", etc.
            symbol = symbol + 'USDT'
...
def get_symbol_selection() -> str:
    """Ask user to select trading symbol for orderbook with flexible input."""
    print("\n" + "="*50)
    ...
    while True:
        try:
            ...
            if len(normalized_symbol) < 6:
                print("Trading symbol too short after normalization. Please try again.")
                continue
...
class MEXCOrderbookMonitorDirectGeneratedProto:
    def __init__(self, symbol, protobuf_modules):
        ...
        # API credentials (will be loaded)
        self.api_key = None
    ...
    def load_api_keys(self):
        ...
        try:
            ...
            if not self.api_key or not self.api_secret:
                self.log_with_timestamp("❌ Missing keys in config/mexc_keys.json")
                return False
            
            self.log_with_timestamp("✅ API keys loaded successfully")
            return True
            
        except Exception as e:
            self.log_with_timestamp(f"❌ Error loading keys: {e}")
            return False
    ...
    def create_listen_key(self):
        """Create listen key."""
        if not self.api_key:
            self.log_with_timestamp("❌ No API key available")
            return False
    ...
    def on_open(self, ws):
        ...
        if self.listen_key:
            self.log_with_timestamp("✅ DIRECT GENERATED_PROTO ORDERBOOK WEBSOCKET CONNECTED WITH LISTEN KEY!")
            self.log_with_timestamp(f"🔗 URL: {self.ws_url}?listenKey={self.listen_key[:20]}...")
        else:
            self.log_with_timestamp("✅ DIRECT GENERATED_PROTO ORDERBOOK WEBSOCKET CONNECTED (PUBLIC)!")
    ...
    def connect_websocket(self):
        ...
        if self.listen_key:
            ...
            self.log_with_timestamp("🔗 CONNECTING WITH LISTEN KEY...")
        else:
            ws_url = self.ws_url
            self.log_with_timestamp("🔗 CONNECTING PUBLIC WEBSOCKET...")
    ...
    def start_monitoring(self):
        ...
        # Create listen key if API keys are available
        if self.api_key:
            if not self.create_listen_key():
                self.log_with_timestamp("❌ Failed to get listen key, continuing without authenticated connection")
        else:
            self.log_with_timestamp("⚠️ No API keys - using public connection only")
...
# ============================================================================
# TEXTUAL UI COMPONENTS - 30/70 LAYOUT WITH GAP + WEIGHTED AVERAGE PRICE
# ============================================================================

class OrderbookConnectionPanel(Static):
    """Panel for displaying WebSocket connection status and debug info - 30% HEIGHT."""
    ...
    def compose(self) -> ComposeResult:
        ...
        with Container():
            yield Static("Orderbook Monitor - DIRECT GENERATED_PROTO", classes="connection-header")
    ...
    def update_connection_info(self, conn_state: dict) -> None:
        ...
        try:
            ...
            display_key = "None"
            if listen_key_full:
                display_key = f"{listen_key_full[:8]}...{listen_key_full[-4:]}"
            
            info_text = f"""[{status_color}]Status: {status}[/{status_color}] | [{mode_color}]Parser: {protobuf_mode}[/{mode_color}] | Symbol: {symbol}
    Connected: {conn_time_str} | Msgs: {messages} (B:{binary_msgs}/J:{json_msgs}) | Reconnects: {reconnects}
    Last Ping: {ping_time_str} | Last Message: {msg_time_str} | Key Extended: {key_extend_str}
    🔑 Key: {display_key} | Created: {key_created_str} | Expires: {key_expires_str}"""
            
            if last_error:
                info_text += f"\n[red]Error: {last_error[:80]}...[/red]"
    ...
    def add_debug_message(self, message: str, msg_type: str = "info") -> None:
        """Add debug message to the 4-line log."""
        try:
...
class OrderbookDisplayPanel(Static):
    """Panel for displaying 20-depth orderbook - 70% HEIGHT WITH WEIGHTED AVERAGE PRICE."""
    ...
    def compose(self) -> ComposeResult:
        """Create the orderbook display layout - SIDE BY SIDE WITH WEIGHTED AVERAGE PRICE."""
    ...
    def on_mount(self) -> None:
        """Initialize orderbook tables with cumulative columns and weighted average price."""
        # Setup bids table with all 6 columns including Avg.Price
        ...
        # Setup asks table with all 6 columns including Avg.Price
    ...
    def update_orderbook(self, orderbook: dict) -> None:
        """Update orderbook display with 20-depth data, cumulative values and weighted average price."""
        if not orderbook:
            return
        ...
        try:
            ...
            # Update bids table (buy orders - highest price first) with all values
            bids_table = self.query_one("#bids-table")
            bids_table.clear()
            ...
            for i, (price, quantity) in enumerate(bids[:20]):
                ...
                try:
                    ...
                    # Calculate weighted average price: Cum.USD / Cum.Qty
            ...
            # Update asks table (sell orders - lowest price first) with all values
            asks_table = self.query_one("#asks-table")
            asks_table.clear()
...
class MEXCOrderbookAppDirectGeneratedProto(App):
    """Main application class - DIRECT GENERATED_PROTO ORDERBOOK VERSION WITH 30/70 LAYOUT."""
    ...
    def compose(self) -> ComposeResult:
        """Create main layout with 30/70 proportions and gap."""
...
# ============================================================================
# MAIN PROGRAM - DIRECT GENERATED_PROTO ORDERBOOK VERSION WITH ALL FIXES
# ============================================================================

def main():
    """Main program entry for DIRECT GENERATED_PROTO orderbook monitor with all fixes."""
    global should_exit, selected_symbol
    
    try:
        print("🔥 MEXC DIRECT GENERATED_PROTO ORDERBOOK MONITOR - FINAL VERSION")
        print("🎯 NO FALLBACKS - STRAIGHT TO GENERATED_PROTO FOLDER")
        print("✅ ALL FIXES: 30/70 layout + 4-line debug + cumulative + weighted avg price + gap")
        print("="*75)
        ...
        # Setup logging
        setup_logging()
        logging.info("Starting DIRECT GENERATED_PROTO MEXC Orderbook Monitor - FINAL VERSION")
        
        # Get symbol selection with improved input handling
        selected_symbol = get_symbol_selection()
        
        print(f"\nStarting DIRECT GENERATED_PROTO orderbook monitor for: {selected_symbol}")
        print("Starting GUI with FINAL layout (30/70 + 4-line debug + cumulative + weighted avg)...")
        
        # Update terminal title
        set_terminal_title(f"MEXC Orderbook Monitor - FINAL - {selected_symbol}")
